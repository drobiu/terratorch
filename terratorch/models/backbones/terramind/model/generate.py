...
def onex_temp_schedule(max_t, min_t, token_schedule, power=0.5, min_linspace=1, max_linspace=100):
    ...
    unscaled_schedule = [(1 - cs) * us for us, cs in zip(unscaled_schedule, schedule_cumsum, strict=False)]
...
def build_chained_generation_schedules(
    ...
    for target_idx in range(len(target_domains)):
        ...
        schedule = [
            ...
            for tok, temp, cfg in zip(token_schedule, temp_schedule, cfg_schedule, strict=False)
        ]
...
class GenerationSampler(nn.Module):
    ...
    def merge_sequences_batched(self, mod_dict, pred_ids, target_mod, text_tokenizer, default_sentinel="[S_1]"):
        ...
        input_dicts = []
        for t, im in zip(tensors, input_masks, strict=False):
        ...
        merged_target_masks = []
        merged_seq_lens = []
        for input_d, pi in zip(input_dicts, pred_ids, strict=False):
    ...
    def multi_guided_maskgit_step_batched(
        ...
        logits = logits_uncond + torch.stack(
            [w * (logits_cond - logits_uncond) for w, logits_cond in zip(cond_weights, logits_cond_all, strict=False)]
        ).sum(dim=0)
